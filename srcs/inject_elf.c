...
int create_shellcode(ELF_datas_64 *elf_datas, Injection_infos *injection_infos)
{
    ...
    if (err != SUCCESS)
        return ERROR;
    printf(COLOR_BOLD_GREEN"[*] "COLOR_BOLD_WHITE"Encryption of the .text section done !\n"COLOR_RESET);
    ...
    close(shellcode_fd);
    printf(COLOR_BOLD_GREEN"[*] "COLOR_BOLD_WHITE"Generation of the assembly source file done !\n"COLOR_RESET);
...
int inject_program_segment(ELF_datas_64 *elf_datas)
{
    ...
    injection_info.to_inject->p_filesz = injection_info.to_inject->p_filesz + injection_info.shellcode_sz;
    injection_info.to_inject->p_memsz = injection_info.to_inject->p_memsz + injection_info.shellcode_sz;
    printf(COLOR_BOLD_GREEN"[*] "COLOR_BOLD_WHITE"Shellcode offset : 0x%lx\n"COLOR_RESET, injection_info.shellcode_off);
    ...
    close(woodyfd);
    printf(COLOR_BOLD_GREEN"[*] "COLOR_BOLD_WHITE"Done ! You can try if everything work as expected.\n"COLOR_RESET);
    return SUCCESS;
}
